...
def main():
    ...
    if not args.skip_extract:
        success = run_step(
            ...
            skip_condition=get_misra_c_extracted_text_path(project_root),
            force=args.force
        )
        if success:
            steps_completed += 1
    ...
    if not args.skip_extract:
        success = run_step(
            ...
            skip_condition=get_fls_index_path(project_root),
            force=args.force
        )
        if success:
            steps_completed += 1
    ...
    section_emb = get_fls_section_embeddings_path(project_root)
    para_emb = get_fls_paragraph_embeddings_path(project_root)
